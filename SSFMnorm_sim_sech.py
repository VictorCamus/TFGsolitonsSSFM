...
EspaiTotal = 0.5 * np.pi # Espai total considerat (unitats L_D)
TempsMàxim = 20 # Temps total considerat (unitats β_2)
Ordre = 4 # Ordre del solitó
...
ax.plot(Tau[idx1:idx2], abs(VectorFuncions[idx3,idx1:idx2]), '--',\
 color='y', label=r"Pols a mitja dispersió ($\xi = \frac{\pi}{4}$)")
ax.plot(Tau[idx1:idx2], abs(VectorFuncions[-1,idx1:idx2]), '-.',\
 color='b', label=r"Pols final ($\xi = \frac{\pi}{2}$)")
ax.grid()
ax.legend(loc="best")
plt.xlabel(r'$\tau$')
plt.ylabel(r'|$u$|')
plt.title(f'NLSE: Solitó Ordre {Ordre}')
